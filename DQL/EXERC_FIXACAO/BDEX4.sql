...
SELECT 
	P.ID_PEDIDO, P.DATA_PEDIDO, P.HORA_PEDIDO, F.NOME_FOR AS NOME_FORNECEDOR
FROM 
	PEDIDOS_COMPRA AS P
INNER JOIN 
	FORNECEDORES AS F
...
SELECT 
	A.NOME_ARMAZEM, P.NOME_PRO, B.QUANTIDADE
FROM 
	PRODUTOS AS P
JOIN 
	PRODUTOS_ARMAZENS AS B
ON 
	P.ID_PRODUTO = B.PRODUTO
JOIN 
	ARMAZENS AS A
ON 
	B.ARMAZEM = A.ID_ARMAZEM
GROUP BY 
	B.ARMAZEM
ORDER BY 
	P.NOME_PRO ASC;
...
SELECT 
	F.NOME_FOR, A.CIDADE_ARMAZEM
FROM 
	FORNECEDORES AS F
JOIN 
	PRODUTOS_FORNECEDORES AS PF
ON 
	F.ID_FORNECEDOR = PF.FORNECEDOR
JOIN 
	PRODUTOS AS P
ON 
	PF.PRODUTO = P.ID_PRODUTO
JOIN 
	PRODUTOS_ARMAZENS AS PA
ON 
	P.ID_PRODUTO = PA.PRODUTO
JOIN 
	ARMAZENS AS A
ON 
	PA.ARMAZEM = A.ID_ARMAZEM
WHERE 
	A.CIDADE_ARMAZEM = "SÃO PAULO";
...
SELECT 
	P.ID_PRODUTO, P.NOME_PRO, P.DESCRICAO, P.PRECO, F.NOME_FOR AS FORNECEDOR
FROM 
	PRODUTOS AS P
JOIN 
	PRODUTOS_FORNECEDORES AS PF
ON 
	P.ID_PRODUTO = PF.PRODUTO
JOIN 
	FORNECEDORES AS F
ON 
	PF.FORNECEDOR = F.ID_FORNECEDOR;

#10. Para cada pedido de compra, mostrar o ID do pedido e a quantidade total de produtos solicitados.
SELECT 
	ID_PEDIDO, SUM(QUANTIDADE)
FROM 
	PEDIDOS_COMPRA
GROUP BY 
	ID_PEDIDO;
...
SELECT 
	F.NOME_FOR,
    ROUND(PF.PRODUTO) AS QNTD_PRODUTOS
FROM 
	FORNECEDORES AS F
JOIN 
	PRODUTOS_FORNECEDORES AS PF
...
ON 
	PA.ARMAZEM = A.ID_ARMAZEM 
WHERE 
	A.CIDADE_ARMAZEM = 'RIO DE JANEIRO';
